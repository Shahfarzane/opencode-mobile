...
const normalizeDirectoryPayload = (
	...
	entries: Array.isArray(result.entries)
		? result.entries.map((entry) => ({
				...
				size: entry.size ?? 0,
				modified:
					(entry as { modified?: string }).modified ?? new Date().toISOString(),
			}))
		: [],
});
...
export const createDesktopFilesAPI = (): FilesAPI => ({
	async listDirectory(path: string): Promise<DirectoryListResult> {
		try {
			const result = await safeInvoke<ListDirectoryResponse>(
				"list_directory",
				{
	...
	async search(payload: FileSearchQuery): Promise<FileSearchResult[]> {
		try {
			...
			const result = await safeInvoke<SearchFilesResponse>(
				"search_files",
				{
	...
	async createDirectory(
		...
		try {
			...
			const result = await safeInvoke<{ success: boolean; path: string }>(
				"create_directory",
				{
					path: normalizedPath,
				},
