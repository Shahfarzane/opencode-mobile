...
async function safeGitInvoke<T>(
	command: string,
	...
	try {
		return await safeInvoke<T>(command, args, {
			...
			onCancel: () => {
				console.warn(
					`[GitAPI] Git operation ${command} did not complete within 120s; it may still be running.`,
				);
			},
		});
	} catch (error) {
...
export const createDesktopGitAPI = (): GitAPI => ({
	...
	async getGitFileDiff(
		...
		const [original, modified] = await safeGitInvoke<[string, string]>(
			"get_git_file_diff",
			{
				directory,
				pathStr: options.path,
	...
	async listGitWorktrees(directory: string): Promise<GitWorktreeInfo[]> {
		return safeGitInvoke<GitWorktreeInfo[]>("list_git_worktrees", {
			directory,
		});
	},
	...
	async removeGitWorktree(
		directory: string,
		payload: GitRemoveWorktreePayload,
	): Promise<{ success: boolean }> {
	...
	async getCurrentGitIdentity(
		directory: string,
	): Promise<GitIdentitySummary | null> {
		try {
			return await safeGitInvoke<GitIdentitySummary>(
	...
	async setGitIdentity(
		...
		const profile = await safeGitInvoke<GitIdentityProfile>(
			"set_git_identity",
			{ directory, profileId },
		);
	...
	async createGitIdentity(
		...
		return safeGitInvoke<GitIdentityProfile>("create_git_identity", {
			profile,
		});
	},
	...
	async updateGitIdentity(
		...
		return safeGitInvoke<GitIdentityProfile>("update_git_identity", {
			id,
			updates,
		});
	},
