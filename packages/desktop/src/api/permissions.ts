...
export const createDesktopPermissionsAPI = (): PermissionsAPI => ({
	async requestDirectoryAccess(
		...
		try {
			...
			const result = await safeInvoke<DirectoryPermissionResult>(
				...
				{ request },
				{
					...
					onCancel: () => {
						console.warn(
							"[PermissionsAPI] Request directory access operation timed out",
						);
					},
				},
			);
	...
	async startAccessingDirectory(path: string): Promise<StartAccessingResult> {
		try {
			const { safeInvoke } = await import("../lib/tauriCallbackManager");
			const result = await safeInvoke<StartAccessingResult>(
				...
				{ path },
				{
					...
					onCancel: () => {
						console.warn(
							"[PermissionsAPI] Start accessing directory operation timed out",
						);
					},
				},
			);
		...
		try {
			...
			const result = await safeInvoke<StartAccessingResult>(
				...
				{ path },
				{
					...
					onCancel: () => {
						console.warn(
							"[PermissionsAPI] Stop accessing directory operation timed out",
						);
					},
				},
			);
