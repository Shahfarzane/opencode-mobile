import { cleanupAllTauriCallbacks, safeInvoke } from "./tauriCallbackManager";
...
async function setupBridge(): Promise<void> {
	try {
		const info = await safeInvoke<ServerInfo>(
			"desktop_server_info",
			{},
			{
				timeout: 10000,
		...
		if (
			...
			try {
				(
					window as { __openchamberCleanup?: () => void }
				).__openchamberCleanup?.();
			} catch (cleanupError) {
				console.warn(
					"[bridge] Cleanup during failed initialization failed:",
					cleanupError,
				);
			}
			delete (window as { __openchamberCleanup?: () => void })
				.__openchamberCleanup;
		}
...
function patchEventSource(origin: string) {
	...
	class DesktopEventSource extends OriginalEventSource {
		constructor(url: string | URL, eventSourceInit?: EventSourceInit) {
			...
			super(
				normalized.startsWith("/") ? `${origin}${normalized}` : normalized,
				eventSourceInit,
			);
		}
	}

	Object.defineProperty(DesktopEventSource, "name", {
		value: "DesktopEventSource",
	});
	Object.setPrototypeOf(
		DesktopEventSource.prototype,
...
function registerDevtoolsShortcut() {
	const handler = (event: KeyboardEvent) => {
		...
		if ((event.metaKey || event.ctrlKey) && event.altKey && key === "i") {
			...
			const devtoolsPromise = safeInvoke(
