...
import { type EventStreamStatus, useUIStore } from "@/stores/useUIStore";
...
const extractSortableId = (id: unknown): string | null => {
	...
	const candidate =
		underscoreIndex >= 0 ? trimmed.slice(underscoreIndex + 1) : trimmed;
...
const messageCache = new Map<
	string,
	{ sessionId: string; message: { info: Message; parts: Part[] } | null }
>();
const getMessageFromStore = (
	sessionId: string,
	messageId: string,
...
export const useEventStream = () => {
	...
	const fallbackDirectory = useDirectoryStore(
	...
	const activeSessionDirectory = React.useMemo(() => {
		...
		try {
			...
			console.warn(
				"Failed to inspect worktree metadata for session directory:",
				error,
			);
		}
		...
		if (
			...
			sessionRecord.directory.trim().length > 0
		) {
	...
	const effectiveDirectory = React.useMemo(() => {
		...
		if (
			typeof fallbackDirectory === "string" &&
			fallbackDirectory.trim().length > 0
		) {
	...
	const setEventStreamStatus = useUIStore(
		(state) => state.setEventStreamStatus,
	);
	...
	const publishStatus = React.useCallback(
		(status: EventStreamStatus, hint?: string | null) => {
			...
			if (streamDebugEnabled()) {
				...
				const message = normalizedHint
					? `${prefix} SSE ${status}: ${normalizedHint}`
					: `${prefix} SSE ${status}`;
	...
	const resyncMessages = React.useCallback(
		(sessionId: string, reason: string) => {
			...
			const task = loadMessages(sessionId)
				.catch((error) => {
					console.warn(
						`[useEventStream] Failed to resync messages (${reason}):`,
						error,
					);
				})
				.finally(() => {
		...
		async (reason: string) => {
			...
			try {
				await Promise.all([
					loadSessions(),
					currentSessionId
						? resyncMessages(currentSessionId, reason)
						: Promise.resolve(),
				]);
			} catch (error) {
	...
	const trackMessage = React.useCallback(
		(
			messageId: string,
			event?: string,
			extraData?: Record<string, unknown>,
	...
	const metadataRefreshTimestampsRef = React.useRef<Map<string, number>>(
		new Map(),
	);
	...
	const sessionCooldownTimersRef = React.useRef<Map<string, NodeJS.Timeout>>(
		new Map(),
	);
	const sessionActivityPhaseRef = React.useRef<
		Map<string, "idle" | "busy" | "cooldown">
	>(new Map());
	const sessionStatusLastRefreshAtRef = React.useRef<number>(0);
	const sessionStatusRefreshInFlightRef = React.useRef<Promise<void> | null>(
		null,
	);
	...
	const requestSessionListRefresh = React.useCallback(() => {
		...
		sessionRefreshTimeoutRef.current = setTimeout(() => {
			...
			try {
				...
				console.warn(
					"Failed to refresh sessions after stream completion:",
					error,
				);
			}
		}, 500);
	...
	const updateSessionActivityPhase = React.useCallback(
		(sessionId: string, phase: "idle" | "busy" | "cooldown") => {
			const storePhase = useSessionStore
				.getState()
				.sessionActivityPhase?.get(sessionId);
			if (storePhase === phase) {
				sessionActivityPhaseRef.current = new Map(
					useSessionStore.getState().sessionActivityPhase ?? new Map(),
				);
				return;
			}
	...
	const refreshSessionActivityStatus = React.useCallback(async () => {
		...
		const resolveSessionDirectoryForStatus = (
			sessionId: string,
		): string | null => {
			try {
			...
			const recordPath = normalizeDirectory(
				(record as { directory?: string | null })?.directory ?? null,
			);
			return recordPath;
		};
	...
	const handleEvent = React.useCallback(
		(event: EventData) => {
			...
			const nonMetadataSessionEvents = new Set([
				"session.abort",
				"session.error",
			]);
			...
			if (!nonMetadataSessionEvents.has(event.type)) {
				const sessionPayload =
					(typeof props.session === "object" && props.session !== null
						? props.session
						: null) ||
					((typeof props.sessionInfo === "object" && props.sessionInfo !== null
						? props.sessionInfo
				...
				if (sessionPayload) {
					...
					const sessionId =
						typeof sessionPayloadAny.id === "string" &&
						sessionPayloadAny.id.length > 0
							? sessionPayloadAny.id
							: typeof sessionPayloadAny.sessionID === "string" &&
									sessionPayloadAny.sessionID.length > 0
								? sessionPayloadAny.sessionID
					...
					if (sessionId) {
						const titleCandidate =
							typeof sessionPayloadAny.title === "string"
								? sessionPayloadAny.title
								: typeof props.title === "string"
									? props.title
									: undefined;

						const summaryCandidate =
							typeof sessionPayloadAny.summary === "object" &&
							sessionPayloadAny.summary !== null
								? (sessionPayloadAny.summary as Session["summary"])
			...
			switch (event.type) {
					...
					if (isDesktopRuntimeRef.current) break;
					{
						...
						const statusObj =
							typeof props.status === "object" && props.status !== null
								...
								: null;
						const statusType =
							typeof statusObj?.type === "string" ? statusObj.type : null;
				...
				case "message.part.updated": {
					...
					const messageInfoSessionId =
						typeof (messageInfo as { sessionID?: unknown }).sessionID ===
						"string"
							? (messageInfo as { sessionID?: string }).sessionID
							: null;

					const resolvedSessionId =
						typeof partExt.sessionID === "string" &&
						(partExt.sessionID as string).length > 0
							? (partExt.sessionID as string)
							: typeof messageInfoSessionId === "string" &&
									messageInfoSessionId.length > 0
					...
					const messageInfoId =
						typeof (messageInfo as { id?: unknown }).id === "string"
							? (messageInfo as { id?: string }).id
							: null;

					const resolvedMessageId =
						typeof partExt.messageID === "string" &&
						(partExt.messageID as string).length > 0
							? (partExt.messageID as string)
							: typeof messageInfoId === "string" && messageInfoId.length > 0
								? messageInfoId
								: typeof props.messageID === "string" &&
										(props.messageID as string).length > 0
					...
					if (!resolvedSessionId || !resolvedMessageId) {
						if (streamDebugEnabled()) {
							console.debug(
								"[useEventStream] Skipping message.part.updated without resolvable session/message id",
								{
									sessionID:
										partExt.sessionID ??
										messageInfoSessionId ??
										props.sessionID,
									messageID:
										partExt.messageID ?? messageInfoId ?? props.messageID,
								},
							);
						}
						break;
					...
					const trimmedHeadMaxId = useSessionStore
						.getState()
						.sessionMemoryState.get(sessionId)?.trimmedHeadMaxId;
					if (trimmedHeadMaxId && !isIdNewer(messageId, trimmedHeadMaxId)) {
						if (streamDebugEnabled()) {
							console.debug(
								"[useEventStream] Skipping message.part.updated for trimmed message",
								{
									sessionId,
									messageId,
									trimmedHeadMaxId,
					...
					if (
						messageInfo &&
						typeof (messageInfo as { role?: unknown }).role === "string"
					) {
						...
						if (existingMessage) {
							const existingRole = (
								existingMessage.info as Record<string, unknown>
							).role;
				...
				case "message.updated": {
					...
					const resolvedSessionId =
						typeof messageExt.sessionID === "string" &&
						(messageExt.sessionID as string).length > 0
							? (messageExt.sessionID as string)
							: typeof props.sessionID === "string" &&
									(props.sessionID as string).length > 0
								? (props.sessionID as string)
								: null;
					...
					const resolvedMessageId =
						typeof messageExt.id === "string" &&
						(messageExt.id as string).length > 0
					...
					if (!resolvedSessionId || !resolvedMessageId) {
						if (streamDebugEnabled()) {
							console.debug(
								"[useEventStream] Skipping message.updated without resolvable session/message id",
								{
					...
					if (trimmedHeadMaxId && !isIdNewer(messageId, trimmedHeadMaxId)) {
						if (streamDebugEnabled()) {
							console.debug(
								"[useEventStream] Skipping message.updated for trimmed message",
								{
									sessionId,
									messageId,
									trimmedHeadMaxId,
					...
					if (isDesktopRuntimeRef.current && streamDebugEnabled()) {
						try {
							const serverParts =
								(props as { parts?: unknown }).parts ||
								(messageExt as { parts?: unknown }).parts ||
								[];
					...
					trackMessage(messageId, "message_updated", {
						role: (messageExt as { role?: unknown }).role,
					});
					...
					if ((messageExt as { role?: unknown }).role === "user") {
						const serverParts =
							(props as { parts?: unknown }).parts ||
							(messageExt as { parts?: unknown }).parts;
						const partsArray = Array.isArray(serverParts)
							? (serverParts as Part[])
							: [];
						...
						if (partsArray.length > 0) {
							for (let i = 0; i < partsArray.length; i++) {
								...
								if ((serverPart as Record<string, unknown>).synthetic === true)
									continue;
								...
								const enrichedPart: Part = {
									...
									sessionID:
										(serverPart as { sessionID?: string })?.sessionID ||
										sessionId,
									messageID:
										(serverPart as { messageID?: string })?.messageID ||
										messageId,
								} as Part;
						...
						trackMessage(messageId, "user_message_created_from_event", {
					...
					const existingStopMarker =
						(existingMessage?.info as { finish?: string } | undefined)
							?.finish === "stop";
					...
					const timeObj =
						(messageExt as { time?: { completed?: number } }).time || {};
					...
					const finish =
						typeof finishCandidate === "string" ? finishCandidate : null;
					const eventHasStopFinish = finish === "stop";

					if (
						(messageExt as { role?: unknown }).role === "assistant" &&
						hasParts
					) {
						const incomingLen = computeTextLength(partsArray);
						const wouldShrink =
							existingLen > 0 &&
							incomingLen + TEXT_SHRINK_TOLERANCE < existingLen;

						if (wouldShrink && !eventHasStopFinish) {
							trackMessage(messageId, "skipped_shrinking_update", {
								incomingLen,
								existingLen,
							});
							break;
						...
						if (isDesktopRuntimeRef.current) {
							const zeroToleranceShrink =
								existingLen > 0 && incomingLen < existingLen;
							...
							if (zeroToleranceShrink && !shrinkAllowed) {
								trackMessage(messageId, "desktop_shrinking_update_suppressed", {
									incomingLen,
									existingLen,
								});
								break;
					...
					if (hasParts && (messageExt as { role?: unknown }).role !== "user") {
						...
						const existingMessageForSession = existingMessages.find(
							(m) => m.info.id === messageId,
						);
						const needsInjection =
							!existingMessageForSession ||
							existingMessageForSession.parts.length === 0;

						trackMessage(
							messageId,
							needsInjection
								? "server_parts_injected"
								: "server_parts_refreshed",
						...
						for (let i = 0; i < partsToInject.length; i++) {
							...
							addStreamingPart(
								...
								messageId,
								enrichedPart,
								(messageExt as { role?: string }).role as string,
							);
							trackMessage(messageId, `server_part_${i}`);
						}
					}

					const messageTime = (message as { time?: { completed?: unknown } })
						.time;
					const completedCandidate = (
						messageTime as { completed?: unknown } | undefined
					)?.completed;
					const hasCompletedTimestamp =
						typeof completedCandidate === "number" &&
						Number.isFinite(completedCandidate);
					...
					if (
						shouldFinalizeAssistantMessage &&
						(message as { role?: string }).role === "assistant"
					) {
						const storeState = useSessionStore.getState();
						const sessionMessages = storeState.messages.get(sessionId) || [];
						...
						const timeCompleted = hasCompletedTimestamp
						...
						if (ENABLE_EMPTY_RESPONSE_DETECTION) {
							const completedMessage = getMessageFromStore(
								sessionId,
								messageId,
							);
							if (completedMessage) {
								const storedParts = Array.isArray(completedMessage.parts)
								...
								for (let i = 0; i < eventParts.length; i++) {
									...
									const alreadyPresent = combinedParts.some(
										(existing) =>
											...
											(existing as { callID?: string }).callID ===
												(normalized as { callID?: string }).callID,
									);
								...
								const hasMeaningfulContent =
									hasTextContent || hasTools || hasReasoning || hasFiles;
								const isEmptyResponse =
									!hasMeaningfulContent && !hasStepMarkers;

								if (
									isEmptyResponse &&
									!emptyResponseToastShownRef.current.has(messageId)
								) {
									emptyResponseToastShownRef.current.add(messageId);
									import("sonner").then(({ toast }) => {
										toast.info("Assistant response was empty", {
											description:
												"Try sending your message again or rephrase it.",
											duration: 5000,
										});
									});
								}
						...
						if (!isDesktopRuntimeRef.current) {
							if (finish === "stop") {
								const currentPhase = useSessionStore
									.getState()
									.sessionActivityPhase?.get(sessionId);
						...
						const rawMessageSessionId = (message as { sessionID?: string })
							.sessionID;
						const messageSessionId: string =
							typeof rawMessageSessionId === "string" &&
							rawMessageSessionId.length > 0
								? rawMessageSessionId
								: sessionId;
				...
				case "session.updated": {
					const candidate =
						typeof props.info === "object" && props.info !== null
							? (props.info as Record<string, unknown>)
							: typeof props.sessionInfo === "object" &&
									props.sessionInfo !== null
								? (props.sessionInfo as Record<string, unknown>)
								: typeof props.session === "object" && props.session !== null
									? (props.session as Record<string, unknown>)
									: props;
					...
					if (sessionId) {
						const timeSource =
							typeof candidate.time === "object" && candidate.time !== null
								? (candidate.time as Record<string, unknown>)
								: typeof props.time === "object" && props.time !== null
									? (props.time as Record<string, unknown>)
									: null;
						const compactingTimestamp =
							timeSource && typeof timeSource.compacting === "number"
								? (timeSource.compacting as number)
								: null;
	...
	const waitForDesktopBridge = React.useCallback(
	...
	const stopStream = React.useCallback(() => {
		if (isCleaningUpRef.current) {
			if (streamDebugEnabled()) {
				console.info(
					"[useEventStream] Already cleaning up, skipping stopStream",
				);
			}
			return;
		}
	...
	const startStream = React.useCallback(
		async (options?: { resetAttempts?: boolean }) => {
			...
			if (isDesktopRuntimeRef.current) {
				...
				if (!bridgeReady) {
					console.warn(
						"[useEventStream] Desktop bridge not ready, falling back to SDK",
					);
				}
			}
			...
			const onOpen = () => {
				...
				const hasBusySessions = Array.from(
					useSessionStore.getState().sessionActivityPhase?.values?.() ?? [],
				).some((phase) => phase === "busy");
				if (hasBusySessions) {
					...
					if (sessionId) {
						setTimeout(() => {
							resyncMessages(sessionId, "sse_reconnected")
								...
								.catch((error) => {
									console.warn(
										"[useEventStream] Failed to resync messages after reconnect:",
										error,
									);
								});
						}, 0);
			...
			if (streamDebugEnabled()) {
				console.info(
					"[useEventStream] Connecting to event source (SDK SSE only):",
					{
						effectiveDirectory,
			...
			if (isCleaningUpRef.current) {
				if (streamDebugEnabled()) {
					console.info(
						"[useEventStream] Skipping subscription due to cleanup in progress",
					);
				}
				return;
			}
			...
			try {
				...
				const compositeUnsub = () => {
					try {
						...
						console.warn(
							"[useEventStream] Error during unsubscribe:",
							cleanupError,
						);
					}
				};
				...
				console.error(
					"[useEventStream] Error during subscription:",
					subscriptionError,
				);
		...
		(hint?: string) => {
			...
			const baseDelay =
				nextAttempt <= 3
					...
					: Math.min(2000 * 2 ** (nextAttempt - 3), 32000);
	...
	React.useEffect(() => {
		...
		let desktopActivityHandler:
			| ((event: CustomEvent<{ sessionId?: string; phase?: string }>) => void)
			| null = null;
		if (isDesktopRuntimeRef.current && typeof window !== "undefined") {
			desktopActivityHandler = (
				event: CustomEvent<{ sessionId?: string; phase?: string }>,
			) => {
				const sessionId =
					typeof event.detail?.sessionId === "string"
						...
						: null;
				const phase =
					typeof event.detail?.phase === "string" ? event.detail.phase : null;
				if (
					sessionId &&
					(phase === "idle" || phase === "busy" || phase === "cooldown")
				) {
			...
			window.addEventListener(
				...
				desktopActivityHandler as EventListener,
			);
		}
		...
		const handleVisibilityChange = () => {
			...
			if (visibilityStateRef.current === "visible") {
				...
				if (pendingResumeRef.current || !unsubscribeRef.current) {
					console.info(
						"[useEventStream] Visibility restored, triggering soft refresh...",
					);
					const sessionId = currentSessionIdRef.current;
					if (sessionId) {
		...
		const handleWindowFocus = () => {
			...
			if (visibilityStateRef.current === "visible") {
				...
				if (pendingResumeRef.current || !unsubscribeRef.current) {
					console.info(
						"[useEventStream] Window focused after pause, triggering soft refresh...",
					);
					const sessionId = currentSessionIdRef.current;
					if (sessionId) {
						requestSessionMetadataRefresh(sessionId);
						resyncMessages(sessionId, "window_focus")
							.then(() =>
								console.info("[useEventStream] Messages refreshed on focus"),
							)
							.catch((err) =>
								console.warn(
									"[useEventStream] Failed to refresh messages:",
									err,
								),
							);
					}
