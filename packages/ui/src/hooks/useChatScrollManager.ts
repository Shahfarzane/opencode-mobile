...
interface UseChatScrollManagerOptions {
	...
	getActiveTurnAnchor: (
		sessionId: string,
	) => { anchorId: string | null; spacerHeight: number } | null;
	isSyncing: boolean;
	isMobile: boolean;
...
const LONG_MESSAGE_THRESHOLD = 0.2;
const LONG_MESSAGE_VISIBLE_PORTION = 0.1;
...
export const useChatScrollManager = ({
	...
	const [pendingAnchorId, setPendingAnchorId] = React.useState<string | null>(
		null,
	);
	...
	const lastFirstMessageIdRef = React.useRef<string | null>(
		sessionMessages.length > 0 ? getMessageId(sessionMessages[0]) : null,
	);
	const lastLastMessageIdRef = React.useRef<string | null>(
		sessionMessages.length > 0
			? getMessageId(sessionMessages[sessionMessages.length - 1])
			: null,
	);
	...
	const calculateAnchorPosition = React.useCallback(
		(anchorElement: HTMLElement, containerHeight: number): number => {
			const messageHeight = anchorElement.offsetHeight;
			const messageTop = anchorElement.offsetTop;
			const isLongMessage =
				messageHeight > containerHeight * LONG_MESSAGE_THRESHOLD;
	...
	const scrollToBottom = React.useCallback(
		(options?: { instant?: boolean; force?: boolean }) => {
			const container = scrollRef.current;
			if (!container) return;
	...
	const scrollToNewAnchor = React.useCallback(
		(messageId: string) => {
	...
	const handleScrollEvent = React.useCallback(
		(event?: Event) => {
			...
			const position =
				(scrollTop + clientHeight / 2) / Math.max(scrollHeight, 1);
			...
			if (shouldRestoreAnchor && persistedAnchor && persistedAnchor.anchorId) {
				...
				window.requestAnimationFrame(() => {
					...
					const anchorElement = container.querySelector(
						`[data-message-id="${persistedAnchor.anchorId}"]`,
					) as HTMLElement | null;
					if (anchorElement) {
						const containerHeight = container.clientHeight;
						const targetScrollTop = calculateAnchorPosition(
	...
	useIsomorphicLayoutEffect(() => {
		...
		if (nextCount > previousCount && previousCount > 0) {
			...
			const firstIdChanged =
				previousFirstId !== null && newFirstId !== previousFirstId;
			const lastIdChanged =
				previousLastId !== null && newLastId !== previousLastId;
			...
			if (wasAppended) {
				const appendedMessages = sessionMessages.slice(
					previousCount,
					nextCount,
				);
	...
	React.useEffect(() => {
		...
		const transitionedToIdle =
			prevPhase !== null && prevPhase !== "idle" && currentPhase === "idle";
	...
	const animationHandlersRef = React.useRef<Map<string, AnimationHandlers>>(
		new Map(),
	);
