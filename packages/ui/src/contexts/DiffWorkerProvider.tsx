...
const WorkerPoolWarmup: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	...
	useEffect(() => {
		...
		const dummyDiff = parseDiffFromFile(dummyFile, {
			...dummyFile,
			contents: "const x = 2;",
			cacheKey: "warmup-file-2",
		});
	...
	useEffect(() => {
		...
		const processChunk = (deadline?: IdleDeadlineLike) => {
			...
			while (index < entries.length) {
				...
				const cacheKey = getPierreCacheKey(
					filePath,
					diff.original,
					diff.modified,
				);
...
export const DiffWorkerProvider: React.FC<DiffWorkerProviderProps> = ({
	children,
}) => {
	...
	return (
		...
		>
			<WorkerPoolWarmup>{children}</WorkerPoolWarmup>
		</WorkerPoolContextProvider>
	);
};
