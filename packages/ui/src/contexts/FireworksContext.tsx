...
const FireworksContext = React.createContext<FireworksContextValue | undefined>(
	undefined,
);

export const FireworksProvider: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	const { isActive, burstKey, triggerFireworks, dismissFireworks } =
		useFireworks();
...
export const useFireworksCelebration = (): FireworksContextValue => {
	...
	if (!context) {
		throw new Error(
			"useFireworksCelebration must be used within FireworksProvider",
		);
	}
	return context;
};
