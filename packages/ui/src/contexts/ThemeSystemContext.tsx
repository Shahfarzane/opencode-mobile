...
const findFallbackThemeId = (variant: "light" | "dark"): string => {
	const fallback = themes.find(
		(candidate) => candidate.metadata.variant === variant,
	);
	...
	if (typeof window !== "undefined") {
		...
		if (
			...
			if (useSystem) {
				...
				if (legacyTheme) {
					themeMode =
						legacyTheme.metadata.variant === "dark" ? "dark" : "light";
...
export function ThemeSystemProvider({
	...
	const [preferences, setPreferences] = useState<ThemePreferences>(() =>
		buildInitialPreferences(defaultThemeId),
	);
	...
	const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
		...
		const existing = (
			window as unknown as { __OPENCHAMBER_VSCODE_THEME__?: Theme }
		).__OPENCHAMBER_VSCODE_THEME__;
	...
	useEffect(() => {
		...
		const applyVSCodeTheme = (theme: Theme) => {
			...
			const variant: ThemeMode =
				theme.metadata.variant === "dark" ? "dark" : "light";
	...
	useEffect(() => {
		...
		const handleSettingsSynced = (event: Event) => {
			...
			setPreferences((prev) => {
				...
				if (detail.useSystemTheme === true) {
					...
					if (
						detail.themeVariant === "dark" ||
						detail.themeVariant === "light"
					) {
				...
				if (
					typeof detail.lightThemeId === "string" &&
					detail.lightThemeId.length > 0
				) {
				...
				if (
					typeof detail.darkThemeId === "string" &&
					detail.darkThemeId.length > 0
				) {
		...
		window.addEventListener(
			"openchamber:settings-synced",
			handleSettingsSynced,
		);
		return () =>
			window.removeEventListener(
				"openchamber:settings-synced",
	...
	const setTheme = useCallback(
		(themeId: string) => {
			const theme = availableThemes.find(
				(candidate) => candidate.metadata.id === themeId,
			);
			if (!theme) {
				return;
			}
			...
			setPreferences((prev) => {
				if (theme.metadata.variant === "dark") {
					if (
						prev.darkThemeId === theme.metadata.id &&
						prev.themeMode === "dark"
					) {
						return prev;
					}
				...
				if (
					prev.lightThemeId === theme.metadata.id &&
	...
	const setLightThemePreference = useCallback(
		(themeId: string) => {
			const theme = availableThemes.find(
				(candidate) =>
					candidate.metadata.id === themeId &&
					candidate.metadata.variant === "light",
			);
			if (!theme) {
				return;
			}
	...
	const setDarkThemePreference = useCallback(
		(themeId: string) => {
			const theme = availableThemes.find(
				(candidate) =>
					candidate.metadata.id === themeId &&
					candidate.metadata.variant === "dark",
			);
			if (!theme) {
				return;
			}
