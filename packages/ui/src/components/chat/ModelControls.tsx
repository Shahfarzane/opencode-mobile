...
import {
	useIsDesktopRuntime,
	useIsVSCodeRuntime,
} from "@/hooks/useRuntimeAPIs";
import { getAgentColor } from "@/lib/agentColors";
import { useDeviceInfo } from "@/lib/device";
import { getEditModeColors } from "@/lib/permissions/editModeColors";
import {
	type BashPermissionSetting,
	calculateEditPermissionUIState,
} from "@/lib/permissions/editPermissionDefaults";
...
const isPrimaryMode = (mode?: string) =>
...
const getModalityIcons = (
	...
	const uniqueValues = Array.from(
		new Set(modalityList.map((item) => normalizeModality(item))),
	);
...
export const ModelControls: React.FC<ModelControlsProps> = ({ className }) => {
	...
	const [activeMobilePanel, setActiveMobilePanel] = React.useState<
		"model" | "agent" | null
	>(null);
	const [mobileTooltipOpen, setMobileTooltipOpen] = React.useState<
		"model" | "agent" | null
	>(null);
	...
	const closeMobileTooltip = React.useCallback(
	...
	const [mobileEditOptionsOpen, setMobileEditOptionsOpen] =
		React.useState(false);
	const [agentMenuOpen, setAgentMenuOpen] = React.useState(false);
	const [desktopEditOptionsOpen, setDesktopEditOptionsOpen] =
		React.useState(false);
	...
	if (
		agentPermissionRaw === "allow" ||
		agentPermissionRaw === "ask" ||
		agentPermissionRaw === "deny" ||
		agentPermissionRaw === "full"
	) {
		agentDefaultEditMode = agentPermissionRaw;
	}

	const editToolConfigured = currentAgent
		? currentAgent.tools?.["edit"] !== false
		: false;
	...
	const agentBashPermission = currentAgent?.permission?.bash as
		| BashPermissionSetting
		| undefined;

	const permissionUiState = React.useMemo(
		() =>
			calculateEditPermissionUIState({
	...
	const { cascadeDefaultMode, modeAvailability, autoApproveAvailable } =
		permissionUiState;

	const selectionContextReady = Boolean(currentSessionId && currentAgentName);
	const sessionMode =
		selectionContextReady && currentSessionId && currentAgentName
			? getSessionAgentEditMode(
					currentSessionId,
					currentAgentName,
					cascadeDefaultMode,
	...
	const editPermissionOptions: Array<{
		mode: EditPermissionMode;
		label: string;
		disabled: boolean;
	}> = [
		{
			mode: "ask",
			label: editModeShortLabels.ask,
			disabled: isModeDisabled("ask"),
		},
		{
			mode: "allow",
			label: editModeShortLabels.allow,
			disabled: isModeDisabled("allow"),
		},
		{
			mode: "full",
			label: editModeShortLabels.full,
			disabled: isModeDisabled("full"),
		},
	];

	const activeEditModeColors = React.useMemo(
		() => getEditModeColors(effectiveEditMode),
		[effectiveEditMode],
	);
	...
	const sizeVariant: "mobile" | "vscode" | "default" = isMobile
		? "mobile"
		: isVSCodeRuntime
			? "vscode"
			: "default";
	const buttonHeight =
		sizeVariant === "mobile" ? "h-9" : sizeVariant === "vscode" ? "h-6" : "h-8";
	const editToggleIconClass =
		sizeVariant === "mobile"
			? "h-5 w-5"
			: sizeVariant === "vscode"
				? "h-4 w-4"
	...
	const renderEditModeIcon = React.useCallback(
		(mode: EditPermissionMode, iconClass = editToggleIconClass) => {
			...
			if (mode === "full") {
				return (
					<RiPencilAiLine className={combinedClassName} style={iconStyle} />
				);
			}
			if (mode === "allow") {
				return (
			...
			if (mode === "deny") {
				return (
					<RiCloseCircleLine className={combinedClassName} style={iconStyle} />
				);
			}
	...
	const handleEditPermissionSelect = React.useCallback(
		(mode: EditPermissionMode) => {
			if (
				editToggleDisabled ||
				!currentSessionId ||
		...
		[
			...
			setSessionAgentEditMode,
			setAgentMenuOpen,
			setDesktopEditOptionsOpen,
			isModeDisabled,
		],
	);

	const currentProvider = getCurrentProvider();
	const models = Array.isArray(currentProvider?.models)
	...
	const costRows = [
		...
		{
			label: "Cache read",
			value: formatCost(currentMetadata?.cost?.cache_read),
		},
		{
			label: "Cache write",
			value: formatCost(currentMetadata?.cost?.cache_write),
		},
	];
	...
	const currentSessionMessageCount = currentSessionId
		? (messages.get(currentSessionId)?.length ?? -1)
		: -1;
	...
	const tryApplyModelSelection = React.useCallback(
		(
			...
			const providerModels = Array.isArray(provider.models)
				? provider.models
				: [];
			const modelExists = providerModels.find(
				(m: ProviderModel) => m.id === modelId,
			);
			if (!modelExists) {
				return "model-missing";
			}
	...
	React.useEffect(() => {
		...
		if (
			!sessionInitializationRef.current ||
			sessionInitializationRef.current.sessionId !== currentSessionId
		) {
			sessionInitializationRef.current = {
		...
		const applySavedSelections = (): "resolved" | "waiting" | "continue" => {
			...
			if (savedAgentName) {
				...
				const savedModel = getAgentModelForSession(
					currentSessionId,
					savedAgentName,
				);
				if (savedModel) {
		...
		const applyFallbackAgent = () => {
			...
			if (fallbackAgent.model?.providerID && fallbackAgent.model?.modelID) {
				tryApplyModelSelection(
					fallbackAgent.model.providerID,
					fallbackAgent.model.modelID,
					fallbackAgent.name,
				);
			}
		};
				...
				if (currentSessionMessageCount > 0) {
					...
					try {
						const discoveredChoices =
							await analyzeAndSaveExternalSessionChoices(
								currentSessionId,
								agents,
							);
						if (isCancelled) {
						...
						if (!isCancelled) {
							console.error(
								"[ModelControls] Error resolving session from messages:",
								error,
							);
						}
					} finally {
						...
						if (
							!isCancelled &&
							refState &&
							refState.sessionId === currentSessionId
						) {
							refState.inFlight = false;
						}
					}
	...
	React.useEffect(() => {
		...
		const preferredSelection = getAgentModelForSession(
			currentSessionId,
			preferredAgent,
		);
		...
		const provider = providers.find(
			(p) => p.id === preferredSelection.providerId,
		);
		if (!provider) {
			return;
		}
		...
		const modelExists = Array.isArray(provider.models)
			? provider.models.some(
					(m: ProviderModel) => m.id === preferredSelection.modelId,
				)
			: false;
		if (!modelExists) {
			return;
			...
			try {
				if (currentAgentName !== prevAgentNameRef.current) {
					...
					if (currentAgentName && currentSessionId) {
						...
						const persistedChoice = getAgentModelForSession(
							currentSessionId,
							currentAgentName,
						);
	...
	const handleProviderAndModelChange = (
		...
		try {
			...
			if (result !== "applied") {
				if (result === "provider-missing") {
					console.error(
						"[ModelControls] Provider not available for selection:",
						providerId,
					);
				} else if (result === "model-missing") {
					console.error("[ModelControls] Model not available for selection:", {
						providerId,
						modelId,
					});
				}
	...
	const getCurrentModelDisplayName = () => {
		...
		const currentModel = models.find(
			(m: ProviderModel) => m.id === currentModelId,
		);
	...
	const getAgentDisplayName = () => {
		if (!currentAgentName) {
			...
			return defaultAgent
				? capitalizeAgentName(defaultAgent.name)
				: "Select Agent";
		}
		const agent = agents.find((a) => a.name === currentAgentName);
		return agent
			? capitalizeAgentName(agent.name)
			: capitalizeAgentName(currentAgentName);
	};
	...
	const toggleMobileProviderExpansion = React.useCallback(
		(providerId: string) => {
			...
			>
				<div className="flex flex-col gap-1.5">
					...
					<div className="rounded-xl border border-border/40 bg-sidebar/30 px-2 py-1.5">
						<div className="typography-micro text-muted-foreground mb-0.5">
							Provider
						</div>
							...
							<div className="flex flex-col gap-1">
								{inputModalityIcons.length > 0 && (
									<div className="flex items-center gap-2">
										<span className="typography-meta text-muted-foreground/80 w-12">
											Input
										</span>
	...
	const renderMobileAgentTooltip = () => {
		if (!isCompact || mobileTooltipOpen !== "agent" || !currentAgent)
			return null;
		...
		const hasCustomPrompt = Boolean(
			currentAgent.prompt && currentAgent.prompt.trim().length > 0,
		);
		const hasModelConfig =
			currentAgent.model?.providerID && currentAgent.model?.modelID;
		const hasTemperatureOrTopP =
			currentAgent.temperature !== undefined || currentAgent.topP !== undefined;

		const getPermissionIcon = (permission?: string) => {
			const mode: EditPermissionMode =
				permission === "full" || permission === "allow" || permission === "deny"
					? permission
					: "ask";
						...
						<div className="rounded-xl border border-border/40 bg-sidebar/30 px-2 py-1.5">
							...
							{hasModelConfig && (
								<div className="typography-meta text-foreground font-medium mb-1">
									{currentAgent.model!.providerID} /{" "}
									{currentAgent.model!.modelID}
								</div>
							)}
					...
					<div className="rounded-xl border border-border/40 bg-sidebar/30 px-2 py-1.5">
						...
						<div className="flex flex-col gap-1">
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-1.5">
									{getPermissionIcon(
										typeof currentAgent.permission?.bash === "string"
											? currentAgent.permission.bash
											: undefined,
									)}
									<span className="typography-meta font-medium text-foreground">
										{getPermissionLabel(
	...
	const renderMobileModelPanel = () => {
		...
		const filteredProviders = providers
			...
			.filter(
				({ matchesProvider, providerModels }) =>
			...
			>
				<div className="flex flex-col gap-2">
												...
												{(metadata?.limit?.context ||
													...
													<div className="typography-micro text-muted-foreground whitespace-nowrap">
														...
														{metadata?.limit?.context && metadata?.limit?.output
															...
															: ""}
														{metadata?.limit?.output
															? `${formatTokens(metadata?.limit?.output)} out`
															: ""}
													</div>
												)}
											</div>
					...
					{filteredProviders.map(({ provider, providerModels }) => {
						...
						const isExpanded =
							expandedMobileProviders.has(provider.id) ||
							normalizedQuery.length > 0;

						return (
							<div
								key={provider.id}
								className="rounded-xl border border-border/40 bg-background/95 overflow-hidden"
							>
								<button
									type="button"
								...
								{isExpanded && providerModels.length > 0 && (
									<div className="flex flex-col border-t border-border/30">
										{providerModels.map((model: ProviderModel) => {
											const isSelected =
												isActiveProvider && model.id === currentModelId;
					...
					{primaryAgents.map((agent) => {
						...
						return (
							...
							>
								<div className="flex items-center gap-1.5">
									<div
										className={cn("h-2 w-2 rounded-full", agentColor.class)}
									/>
									<span
											...
											<button
												...
												style={
													isSelected && optionColors
														? {
																backgroundColor:
																	optionColors.background ?? undefined,
															}
														: undefined
												}
											>
							...
							>
								{currentProviderId ? (
									<>
										...
										/>
										<RiPencilAiLine
											className={cn(controlIconSize, "text-primary/60 hidden")}
										/>
									</>
								) : (
									<RiPencilAiLine
										className={cn(controlIconSize, "text-muted-foreground")}
									/>
								)}
								<span
					...
					<DropdownMenuContent className="max-w-[300px]">
						...
						{favoriteModelsList.length > 0 && (
							<DropdownMenuSub>
								...
								>
									<ScrollableOverlay outerClassName="max-h-[320px] min-w-[200px]">
										{favoriteModelsList.map(
											({ model, providerID, modelID }) => {
												...
												const uniqueModalityIcons = Array.from(
													new Map(
														modalityIcons.map((icon) => [icon.key, icon]),
													).values(),
												...
												const contextTokens = formatTokens(
													metadata?.limit?.context,
												);
												const outputTokens = formatTokens(
													metadata?.limit?.output,
												);
							...
							<DropdownMenuSub>
								...
								>
									<ScrollableOverlay outerClassName="max-h-[320px] min-w-[200px]">
										{recentModelsList.map(({ model, providerID, modelID }) => {
											const metadata = getModelMetadata(providerID, modelID);
											const capabilityIcons = getCapabilityIcons(metadata).map(
												(icon) => ({
													...icon,
													id: `cap-${icon.key}`,
												}),
				...
				<div className="flex min-w-[200px] flex-col gap-2.5">
					...
					<div className="flex flex-col gap-1">
						...
						<div className="flex items-center gap-3">
							<span className="typography-meta text-muted-foreground/80 w-16">
								Edit
							</span>
							<div className="flex items-center gap-1.5">
								{getPermissionIcon(currentAgent.permission?.edit)}
									...
									>
										<RiAiAgentLine
											...
											style={
												currentAgentName
													? {
															color: `var(${getAgentColor(currentAgentName).var})`,
														}
													: undefined
											}
										/>
											...
											{editPermissionOptions.map((option) => {
												...
												return (
													<button
														...
														disabled={option.disabled}
														onClick={() =>
															handleEditPermissionSelect(option.mode)
														}
														className={cn(
