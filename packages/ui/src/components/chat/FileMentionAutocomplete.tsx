...
>(({ searchQuery, onFileSelect, onClose }, ref) => {
	...
	const fuzzyScore = React.useCallback(
		(query: string, candidate: string): number | null => {
				...
				const ranked = normalizedQueryLower
					? (hits
							.map((file) => {
								...
								return score === null
									? null
									: { file, score, labelLength: label.length };
							})
					...
					: hits.map((file) => ({
							file,
							score: 0,
							labelLength: (file.relativePath || file.name || file.path).length,
						}));
				...
				{loading ? (
					...
					<div className="pb-2">
						{files.map((file, index) => {
							...
							const displayPath = truncatePathMiddle(relativePath, {
								maxLength: 45,
							});
