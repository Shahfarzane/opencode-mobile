...
import { RiCheckLine, RiDownloadLine, RiFileCopyLine } from "@remixicon/react";
...
const withStableStringId = <T extends object>(value: T, id: string): T => {
	const existingPrimitive = (value as Record<symbol, unknown>)[
		Symbol.toPrimitive
	];
...
const getMarkdownShikiThemes = (): readonly [
	string | object,
	string | object,
] => {
...
const useMarkdownShikiThemes = (): readonly [
	string | object,
	string | object,
] => {
	...
	React.useEffect(() => {
		...
		window.addEventListener(
			"openchamber:vscode-shiki-themes",
...
const TableCopyButton: React.FC<{
...
const TableDownloadButton: React.FC<{
	tableRef: React.RefObject<HTMLDivElement | null>;
}> = ({ tableRef }) => {
	const [showMenu, setShowMenu] = React.useState(false);
	const menuRef = React.useRef<HTMLDivElement>(null);
	...
	const handleDownload = (format: "csv" | "markdown") => {
		...
		try {
			const data = extractTableData(tableEl);
			const content =
				format === "csv" ? tableToCSV(data) : tableToMarkdown(data);
...
const CodeBlockWrapper: React.FC<CodeBlockWrapperProps> = ({
	...
	const normalizedStyle = React.useMemo<React.CSSProperties | undefined>(() => {
		...
		const bg = normalizeDeclarationString(
	...
	return (
		<div className="group relative" ref={codeRef}>
			<pre {...props} className={cn(className)} style={normalizedStyle}>
				{children}
			</pre>
...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
	...
	if (isAnimated) {
		return (
			<FadeInOnReveal key={componentKey}>{markdownContent}</FadeInOnReveal>
		);
	}
