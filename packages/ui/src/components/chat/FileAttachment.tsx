...
export const FileAttachmentButton = memo(() => {
	...
	const attachFiles = async (files: FileList | File[]) => {
		...
		if (attachedCount > 0) {
			toast.success(
				`Attached ${attachedCount} file${attachedCount > 1 ? "s" : ""}`,
			);
		}
	};
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to pick files in VS Code",
			);
		}
	};
...
const FileChip = memo(({ file, onRemove }: FileChipProps) => {
	...
	return (
		<div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-muted/30 border border-border/30 rounded-xl typography-meta">
			...
			{getFileIcon()}
			<span title={file.serverPath || displayName}>{displayName}</span>
...
export const MessageFilesDisplay = memo(
	({ files, onShowPopup }: MessageFilesDisplayProps) => {
		const fileItems = files.filter(
			(f) => f.type === "file" && (f.mime || f.url),
		);
		...
		const imageFiles = fileItems.filter(
			(f) => f.mime?.startsWith("image/") && f.url,
		);
					...
					<div className="flex flex-wrap gap-2">
						{otherFiles.map((file, index) => (
							...
							>
								...
								<span>{extractFilename(file.filename)}</span>
							</div>
						))}
					</div>
				)}
								...
								return (
									<Tooltip key={`img-${index}`} delayDuration={1000}>
										...
										<TooltipContent
											side="top"
											sideOffset={6}
											className="typography-meta px-2 py-1"
										>
											{filename}
										</TooltipContent>
									</Tooltip>
