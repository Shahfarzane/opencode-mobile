...
const DETAILED_DEFAULT_TOOLS = new Set([
	"task",
	"edit",
	"multiedit",
	"write",
...
const isDetailedDefaultTool = (toolName: unknown): boolean =>
	...
	DETAILED_DEFAULT_TOOLS.has(toolName.toLowerCase());

function useStickyDisplayValue<T>(
	value: T | null | undefined,
): T | null | undefined {
	const ref = React.useRef<{ hasValue: boolean; value: T | null | undefined }>({
		hasValue: false,
		value: undefined as T | null | undefined,
	});
...
const ChatMessage: React.FC<ChatMessageProps> = ({
	...
	const sessionState = useSessionStore(
		useShallow((state) => ({
			...
			isStreamingMessage: (() => {
				...
				return (
					(state.streamingMessageIds.get(sessionId) ?? null) === message.info.id
				);
			})(),
	...
	React.useEffect(() => {
		if (currentSessionId) {
			MessageFreshnessDetector.getInstance().recordSessionStart(
				currentSessionId,
			);
		}
	}, [currentSessionId]);
	...
	const messageRole = React.useMemo(
		() => deriveMessageRole(message.info),
		[message.info],
	);
	...
	const previousUserMetadata = React.useMemo(() => {
		...
		const resolvedAgent =
			typeof mode === "string" && mode.trim().length > 0 ? mode : undefined;
	...
	const providerID = React.useMemo(() => {
		...
		if (
			typeof messageProviderID === "string" &&
			messageProviderID.trim().length > 0
		) {
	...
	const displayProviderIDValue = useStickyDisplayValue<string>(
	...
	const effectiveExpandedTools = React.useMemo(() => {
		...
		if (turnGroupingContext?.isFirstAssistantInTurn) {
			for (const activity of turnGroupingContext.activityParts) {
				...
				const toolPart = activity.part as unknown as {
					id?: string;
					tool?: unknown;
				};
	...
	const agentMention = React.useMemo(() => {
		...
		const name =
			typeof partWithName.name === "string" ? partWithName.name : undefined;
		if (!name) {
			return undefined;
		}
		const rawValue =
	...
	const shouldAnimateMessage = React.useMemo(() => {
		...
		return freshnessDetector.shouldAnimateMessage(
			message.info,
			currentSessionId || message.info.sessionID,
		);
	...
	const assistantSummaryFromStore = useMessageStore((state) => {
		...
		const userMsg = sessionMessages.find(
			(entry) => entry.info?.id === userMessageIdForTurn,
		);
		if (!userMsg) return undefined;
		const summary = (
			userMsg.info as {
				summary?: { body?: string | null | undefined } | null | undefined;
			}
		).summary;
	...
	const assistantSummaryCandidate =
		typeof turnGroupingContext?.summaryBody === "string" &&
		turnGroupingContext.summaryBody.trim().length > 0
	...
	if (
		...
		assistantSummaryCandidate.trim().length > 0
	) {
	...
	const assistantErrorText = React.useMemo(() => {
		...
		const errorInfo = (message.info as { error?: unknown } | undefined)
			?.error as
		...
		if (isUser) {
			const textParts = displayParts
				.filter(
					(
						part,
					): part is Part & { type: "text"; text?: string; content?: string } =>
						part.type === "text",
				)
				.map((part) => {
	...
	const isAnimationSettled = Boolean(
		getMessageInfoProp(message.info, "animationSettled"),
	);
	...
	const allowAnimation =
		shouldAnimateMessage && !isAnimationSettled && !isStreamingPhase;
					...
					{isUser ? (
						<FadeInOnReveal>
							<div className="flex justify-end">
								<div className="max-w-[85%] rounded-xl rounded-br-xs bg-primary/10 dark:bg-primary/8 px-3 py-2">
									<MessageBody
