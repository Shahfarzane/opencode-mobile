...
const getToolDescription = (
	...
	if (
		...
		return isMobile
			? input.description.substring(0, 40)
...
const ToolScrollableSection: React.FC<ToolScrollableSectionProps> = ({
	...
	<ScrollableOverlay
		outerClassName={cn(
			"w-full min-w-0 flex-none overflow-hidden",
			maxHeightClass,
			outerClassName,
		)}
		className={cn(
			"tool-output-surface p-2 rounded-xl w-full min-w-0 border border-border/20 bg-transparent",
			className,
		)}
...
const TaskToolSummary: React.FC<{
	...
	const visibleEntries = isExpanded
		? completedEntries
		: completedEntries.slice(-6);
	const hiddenCount = Math.max(
		0,
		completedEntries.length - visibleEntries.length,
	);
				...
				>
					<div className="w-full min-w-0 space-y-1">
						...
						{visibleEntries.map((entry, idx) => {
							...
							return (
								<div
									key={entry.id ?? `${toolName}-${idx}`}
									className="flex items-center gap-2 min-w-0"
								>
									<span className="flex-shrink-0 text-foreground/80">
										{getToolIcon(toolName)}
									</span>
									<span className="typography-meta text-foreground/80 flex-shrink-0">
										{displayName}
									</span>
									<span className="typography-meta text-muted-foreground/70 truncate">
										{label}
									</span>
								</div>
							);
...
const DiffPreview: React.FC<DiffPreviewProps> = ({
	...
	<div className="typography-code px-1 pb-1 pt-0 space-y-0">
		{parseDiffToUnified(diff).map((hunk, hunkIdx) => (
			<div
				key={hunkIdx}
				className="-mx-1 px-1 border-b border-border/20 last:border-b-0"
			>
...
const WriteInputPreview: React.FC<WriteInputPreviewProps> = ({
	content,
	syntaxTheme,
	filePath,
	displayPath,
		...
		<div className="w-full min-w-0">
			...
			<div className="space-y-0">
				{lines.map((line, lineIdx) => (
					<div
						key={lineIdx}
						className="typography-code font-mono px-2 py-0.5 flex -mx-1"
					>
						<span className="text-muted-foreground/60 w-8 flex-shrink-0 text-right pr-2 self-start select-none">
							{lineIdx + 1}
						</span>
...
const ToolExpandedContent: React.FC<ToolExpandedContentProps> = ({
	...
	const diffContent =
		typeof metadata?.diff === "string" ? (metadata.diff as string) : null;
	const writeFilePath =
		part.tool === "write"
			? typeof input?.filePath === "string"
				? input.filePath
		...
		part.tool === "write"
			? typeof (input as { content?: unknown })?.content === "string"
				...
				: typeof (input as { text?: unknown })?.text === "string"
					...
					: null
			: null;
	const shouldShowWriteInputPreview =
		part.tool === "write" && !!writeInputContent;
	const isWriteImageFile = writeFilePath ? isImageFile(writeFilePath) : false;
	const writeDisplayPath = shouldShowWriteInputPreview
		? writeFilePath
			? getRelativePath(writeFilePath, currentDirectory, isMobile)
			: "New file"
		: null;
	...
	const renderResultContent = () => {
		...
		if (part.tool === "grep" && hasStringOutput) {
			const grepOutput = renderGrepOutput(outputString, isMobile, {
				unstyled: true,
			});
			return renderScrollableBlock(
				grepOutput ?? (
		...
		if (part.tool === "glob" && hasStringOutput) {
			const globOutput = renderGlobOutput(outputString, isMobile, {
				unstyled: true,
			});
			return renderScrollableBlock(
				globOutput ?? (
		...
		if (
			(part.tool === "web-search" ||
				part.tool === "websearch" ||
				part.tool === "search_web") &&
			hasStringOutput
		) {
			const webSearchContent = renderWebSearchOutput(
				outputString,
				syntaxTheme,
				{ unstyled: true },
			);
		...
		if (
			...
			((!hasStringOutput && diffContent) ||
				outputString.trim().length === 0 ||
				hasLspDiagnostics(outputString)) &&
			diffContent
		) {
			return renderScrollableBlock(
				<DiffPreview
		...
		if (hasStringOutput && outputString.trim()) {
			if (part.tool === "read") {
				const formattedOutput = formatEditOutput(
					outputString,
					part.tool,
					metadata,
				);
				...
				return renderScrollableBlock(
					<div className="typography-code w-full min-w-0 space-y-1">
						{lines.map((line: string, idx: number) => {
							...
							const shouldShowLineNumber =
								!isInfo && (limit === undefined || idx < limit);

							return (
								<div
									key={idx}
									className={cn(
										"typography-code font-mono flex w-full min-w-0",
										isInfo && "text-muted-foreground/70 italic",
									)}
								>
									...
									<div className="flex-1 min-w-0">
										{isInfo ? (
											...
											<SyntaxHighlighter
												style={syntaxTheme}
												language={detectLanguageFromOutput(
													formattedOutput,
													part.tool,
...
const ToolPart: React.FC<ToolPartProps> = ({
	...
	const effectiveTimeStart = isTaskTool
		? (pinnedTaskTimeRef.current.start ?? time?.start)
		: time?.start;
	const effectiveTimeEnd = isTaskTool
		? (pinnedTaskTimeRef.current.end ?? time?.end)
		: time?.end;
	...
	const taskSummaryEntries = React.useMemo<TaskToolSummaryEntry[]>(() => {
		...
		return candidate.filter(
			(entry): entry is TaskToolSummaryEntry =>
				typeof entry === "object" && entry !== null,
		) as TaskToolSummaryEntry[];
	}, [isTaskTool, metadata]);
	...
	return (
		<div className="my-1">
				...
				<div className="flex items-center gap-2 flex-shrink-0">
					...
					>
						...
						<div
							...
							style={
								!isTaskTool && isError ? { color: "var(--status-error)" } : {}
							}
						>
			...
			{}
			{isTaskTool &&
			(taskSummaryEntries.length > 0 || isActive || isFinalized) ? (
				<TaskToolSummary
					...
					output={
						typeof stateWithData.output === "string"
