...
const ReasoningPart: React.FC<ReasoningPartProps> = ({
	...
	const textContent = React.useMemo(
		() => cleanReasoningText(rawText),
		[rawText],
	);

	const timeInfo =
		"time" in part ? (part.time as { start: number; end?: number }) : null;
	if (!timeInfo?.end) {
		return null;
	}
...
export const formatReasoningText = (text: string): string =>
	cleanReasoningText(text);
