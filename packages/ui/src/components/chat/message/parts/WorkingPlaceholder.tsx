...
export function WorkingPlaceholder({
	...
	const [displayedPermission, setDisplayedPermission] =
		useState<boolean>(false);

	// Refs for timing
	const statusShownAtRef = useRef<number>(0);
	const queuedStatusRef = useRef<{ text: string; permission: boolean } | null>(
		null,
	);
	const processQueueTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
