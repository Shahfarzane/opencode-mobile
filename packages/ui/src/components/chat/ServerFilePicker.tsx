...
export const ServerFilePicker: React.FC<ServerFilePickerProps> = ({
	...
	const sortDirectoryItems = React.useCallback(
		(items: FileInfo[]) =>
	...
	const mapFilesystemEntries = React.useCallback(
		(
			dirPath: string,
			...
			sortDirectoryItems(
				entries
					...
					.map((item) => {
						...
						const extension =
							!item.isDirectory && name.includes(".")
	...
	const loadDirectory = React.useCallback(
		async (dirPath: string) => {
			...
			try {
				...
				const items = mapFilesystemEntries(
					dirPath,
					entries.map((entry) => ({
						name: entry.name,
						path: entry.path,
	...
	const handleConfirm = async () => {
		...
		const searchFileMap = new Map(
			searchResults.map((file) => [file.path, file]),
		);

		const selected = Array.from(selectedFiles)
			.map(
				(filePath) => treeFileMap.get(filePath) ?? searchFileMap.get(filePath),
			)
	...
	const renderFileItem = (file: FileInfo, level: number) => {
		const rawLabel = isSearchActive
			...
			: file.name;
		const shouldCompact =
			isSearchActive && rawLabel.includes("/") && rawLabel.length > 45;
		...
		const row = (
			...
			>
				<div className="flex flex-1 items-center justify-start gap-1">
					...
					<span
						className="flex-1 truncate text-foreground text-left max-w-[360px]"
						aria-label={rawLabel}
					>
						{displayLabel}
					</span>
	...
	const renderFileTree = (file: FileInfo, level: number): React.ReactNode => {
		...
		const isLoadingChildren =
			isDirectory &&
			isExpanded &&
			inFlightDirsRef.current.has(file.path) &&
			children.length === 0;
		...
		return (
			<div key={file.path}>
				...
				{isDirectory && isExpanded && children.length > 0 && (
					<div>{children.map((child) => renderFileTree(child, level + 1))}</div>
				)}
	...
	const pickerBody = (
		<>
			<div className="px-3 py-2 border-b shrink-0">
				<div className="font-medium typography-ui-label text-foreground">
