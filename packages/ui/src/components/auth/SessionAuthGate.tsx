import { RiLoader4Line, RiLockLine, RiLockUnlockLine } from "@remixicon/react";
...
const AuthShell: React.FC<{ children: React.ReactNode }> = ({ children }) => (
	...
	>
		<div
			...
			style={{
				background:
					"radial-gradient(120% 140% at 50% -20%, var(--surface-overlay) 0%, transparent 68%)",
			}}
		/>
		<div
		...
		<div className="flex flex-col items-center gap-6 text-center">
			<div className="space-y-2">
				...
				<p className="typography-meta text-muted-foreground max-w-xs">
					We couldn't verify the UI session. Check that the service is running
					and try again.
				</p>
			</div>
...
export const SessionAuthGate: React.FC<SessionAuthGateProps> = ({
	...
	const [state, setState] = React.useState<GateState>(() =>
		skipAuth ? "authenticated" : "pending",
	);
				...
				<div className="flex flex-col items-center gap-6 w-full max-w-xs">
					...
					>
						<div className="flex items-center gap-2">
							<div className="relative flex-1">
								...
								<Input
									...
									aria-describedby={
										errorMessage ? "oc-ui-auth-error" : undefined
									}
						...
						{errorMessage && (
							<p
								id="oc-ui-auth-error"
								className="typography-meta text-destructive"
							>
								{errorMessage}
							</p>
						)}
