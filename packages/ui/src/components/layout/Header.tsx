...
export const FixedSessionsButton: React.FC = () => {
	...
	const [isDesktopApp, setIsDesktopApp] = React.useState<boolean>(() => {
		...
		return (
			typeof (window as typeof window & { opencodeDesktop?: unknown })
				.opencodeDesktop !== "undefined"
		);
	});
	...
	const headerIconButtonClass =
		"app-region-no-drag inline-flex h-9 w-9 items-center justify-center gap-2 p-2 typography-ui-label font-medium text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:pointer-events-none disabled:opacity-50 hover:text-foreground";
...
export const Header: React.FC = () => {
	...
	const handleActiveTabDragStart = React.useCallback(
		async (e: React.MouseEvent) => {
			if (e.button !== 0) {
				return;
			}
			...
			<React.Fragment key={tab.id}>
				<button
					...
					className={cn(
						...
						isActive &&
							"after:absolute after:bottom-[-1px] after:left-0 after:right-0 after:h-[2px] after:bg-background",

						isActive &&
							isChatTab &&
							isSidebarOpen &&
							"before:absolute before:bottom-[-1px] before:right-full before:h-px before:bg-[var(--interactive-border)] before:w-[var(--sidebar-w)]",
