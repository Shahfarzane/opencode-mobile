...
import { FixedSessionsButton, Header } from "./Header";
...
export const MainLayout: React.FC = () => {
	...
	React.useEffect(() => {
		...
		const clearKeyboardAvoidTarget = () => {
			...
			keyboardAvoidTarget.style.setProperty(
				"--oc-keyboard-avoid-offset",
				"0px",
			);
		...
		const resolveKeyboardAvoidTarget = (active: HTMLElement | null) => {
			...
			const markedTarget = active.closest(
				"[data-keyboard-avoid]",
			) as HTMLElement | null;
			if (markedTarget) {
				return markedTarget;
		...
		const updateVisualViewport = () => {
			...
			const avoidTarget = isTextTarget
				? resolveKeyboardAvoidTarget(active)
				: null;
			...
			if (!isMobile || !avoidTarget || !active) {
				...
				const avoidOffset =
					overlap > clearance && keyboardInset > 0
						...
						<div className="flex flex-1 flex-col overflow-hidden relative">
							...
							<div
								className={cn(
									...
									(isSettingsActive || isMultiRunLauncherOpen) && "invisible",
								)}
							>
								<Header />
