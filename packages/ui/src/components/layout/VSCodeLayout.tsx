...
export const VSCodeLayout: React.FC = () => {
	...
	const newSessionDraftOpen = useSessionStore((state) =>
	...
	>(
		() =>
			(typeof window !== "undefined"
				? ((window as { __OPENCHAMBER_CONNECTION__?: { status?: string } })
						.__OPENCHAMBER_CONNECTION__?.status as
						| "connecting"
						| "connected"
						| "error"
						| "disconnected"
	...
	const [hasInitializedOnce, setHasInitializedOnce] = React.useState<boolean>(
		() => configInitialized,
	);
	...
	React.useEffect(() => {
		const handler = (event: Event) => {
			const detail = (event as CustomEvent<{ status?: string; error?: string }>)
				.detail;
	...
	React.useEffect(() => {
		const hydrateMessages = async () => {
			...
			const hasMessages =
				messages.has(currentSessionId) &&
				(messages.get(currentSessionId)?.length || 0) > 0;
			if (!hasMessages) {
				try {
	...
	const usesMobileLayout =
		containerWidth > 0 && containerWidth < MOBILE_WIDTH_THRESHOLD;

	return (
		<div
			ref={containerRef}
			className="h-full w-full bg-background text-foreground flex flex-col"
		>
...
const VSCodeHeader: React.FC<VSCodeHeaderProps> = ({
	...
	const limits = (
		currentModel?.limit && typeof currentModel.limit === "object"
			? currentModel.limit
			: null
	) as { context?: number; output?: number } | null;
	...
	return (
		<div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-background shrink-0">
			...
			<h1
				className="text-sm font-medium truncate flex-1 h-9 w-9 items-center justify-center p-2"
				title={title}
			>
				{title}
			</h1>
